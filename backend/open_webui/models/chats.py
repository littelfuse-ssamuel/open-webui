    ...
    def delete_chat_by_id(self, id: str, db: Optional[Session] = None) -> bool:
        ...
        try:
            ...
            with get_db_context(db) as db:
                ...
                success = True and self.delete_shared_chat_by_chat_id(id, db=db)

                if success:
                    log.info(
                        f"Successfully deleted chat {id} with {cleanup_results['files_deleted']} files"
                    )
    ...
    def delete_chat_by_id_and_user_id(
        ...
        try:
            ...
            with get_db_context(db) as db:
                ...
                if success:
                    log.info(
                        f"Successfully deleted chat {id} by user {user_id} with {cleanup_results['files_deleted']} files"
                    )
    ...
    def delete_chats_by_user_id(
        ...
        try:
            ...
            for chat in user_chats:
                ...
                total_cleanup_stats["files_processed"] += cleanup_results[
                    "files_processed"
                ]
                total_cleanup_stats["files_deleted"] += cleanup_results["files_deleted"]
                total_cleanup_stats["images_cleaned"] += cleanup_results[
                    "images_cleaned"
                ]
