<script lang="ts">
	...
	// onSourceClick is passed from parent but not currently used in this component
	// Keeping it for API consistency with parent component expectations
	 ...
	 * Note: This assumes well-formed markdown generated by the filter.
	 * Does not handle escaped brackets or complex edge cases.
	 */
...
{#if token.type === 'html'}
	...
	{:else if html && html.includes('citation-image')}
		...
		{#if imageData}
			<!-- Render with Image component for max-width constraint and click-to-enlarge -->
			<div class="citation-image">
				<Image src={imageData.src} alt={imageData.alt} />
			</div>
		{:else}
			<!-- Fallback: render original HTML if parsing fails -->
			{@html html}
		{/if}
	{:else}
		{token.text}
